...
from . import views
from LO.views import LOView, courseAssessmentPage, ListCourseAssessmentPage, redirectCourseAssessment
from User.views import logoutUser
from User.decorators import allowed_users

urlpatterns = [
    		url(r'^logout/$', logoutUser, name='logout'),
			url(r'^OMM/$', allowed_users(['dosen'])(LOView.as_view()), name='OMM'),
            ...
            url(r'^KomponenNilai/$', allowed_users(['dosen'])(views.DistribusiKomponenNilaiView.as_view()), name='KomponenNilai'),
            url(r'^KomponenNilai/(?P<year>[0-9]+)/(?P<semester>[0-9]+)/(?P<course_id>[A-Z0-9]+)/(?P<section_id>[0-9]+)/$', views.FormsDistribusiNilai, name='FormKomponen'),
            url(r'^KomponenNilai/(?P<year>[0-9]+)/(?P<semester>[0-9]+)/(?P<course_id>[A-Z0-9]+)/(?P<section_id>[0-9]+)/Result/$', views.SubmitView, name='Submit'),
			...
			re_path(r'^CourseAssessment/$', ListCourseAssessmentPage, name='CourseAssessmentList'),
			re_path(r'^CourseAssessment/Submit/$', redirectCourseAssessment, name='SubmitCourseAssessmentList'),
